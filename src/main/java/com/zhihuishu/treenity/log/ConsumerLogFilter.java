...
//		Logger log = LoggerFactory.getLogger("dubbo.consumer." + inter.getName()) ;	// 日志实例
		LoggerTemplate log = LoggerTemplate.getInstance("dubbo.consumer." + inter.getName());
...
//		MDCLogger mdcLogger = MDCLogger.proxy(log ,builder.build()) ;
		builder.build();
		if(ex != null) {
			log.error("接口执行失败，服务器信息：[remoteHost:{}、port:{}、pid:{}、protocol:{}、timeout:{}、version:{}]，请求参数：{}，消息：{}" 
					,host ,port ,pid ,protocol ,timeout ,version ,parametes ,ex.getMessage());
			throw new RpcException(ex) ;
		}
		log.info("接口执行成功，服务器信息：[remoteHost:{}、port:{}、pid:{}、protocol:{}、timeout:{}、version:{}]，请求参数：{}"
