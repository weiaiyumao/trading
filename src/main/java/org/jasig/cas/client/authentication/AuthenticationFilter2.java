...
public class AuthenticationFilter2 extends AbstractCasFilter {
    ...
    // 过滤的请求（不用请求CAS）
 		private String[] filtratePatterns;
    ...
    protected void initInternal(final FilterConfig filterConfig) throws ServletException {
        if (!isIgnoreInitConfiguration()) {
         ...
         // 过滤的请求
		String[] filtrateUrls = parseExcludePatterns(getPropertyFromInitParams(filterConfig, "filtratePatterns", "false"));
		setFiltratePatterns(filtrateUrls);
        }
    }
    ...
    public final void doFilter(final ServletRequest servletRequest, final ServletResponse servletResponse,
        ...
        // 过滤.json请求
        if(StringUtils.endsWithIgnoreCase(request.getRequestURI(), ".json")) {
...
//      // 为了适应多域名情况，不从web.xml的serverName配置中获取域名，改用当前请求地址
        String serverPrefix = request.getScheme() + "://" + request.getServerName() 
				+ (request.getServerPort() == 80 ? "" : (":" + request.getServerPort())) ;
        this.setServerName(serverPrefix);	// 替换掉配置中的serverName属性
        final String serviceUrl = constructServiceUrl(request, response);
//		final String serviceUrl = CommonUtils.constructServiceUrl(request, response, null, serverPrefix, "ticket", true) ;
    ...
    // 处理需要过滤的请求
 	private boolean isfiltratedUrl(HttpServletRequest request) {
 		if(null == filtratePatterns) {
 			return false;
 		}
 		
 		String servletPath = request.getServletPath();
 		for (String pattern : this.filtratePatterns) {
 			...
 			if (pattern.startsWith("/") && pos != -1 && pos == pos2) {
 				...
 				if (pattern.endsWith("*")) {
 					...
 					if (servletPath.startsWith(prefix) && servletPath.endsWith(suffix)) {
 						return true;
 					}
 				}
 			}
	...
	public void setFiltratePatterns(String[] filtratePatterns) {
		this.filtratePatterns = filtratePatterns;
	}
}
